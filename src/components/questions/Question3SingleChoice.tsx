...
export function Question3SingleChoice({ question }: Question3Props) {
  ...
  return (
    <div className="question-container">
      ...
      <fieldset className="single-choice-options">
        <legend className="visually-hidden">{question.text}</legend>
        {question.options.map((option, index) => (
          <label
            key={index}
            className={`choice-option ${selectedIndex === index ? 'selected' : ''}`}
          >
            <input
              type="radio"
